...
```
Tools: `vault_recall` (what the team knows about a topic, one bundle), `vault_search` (BM25 + semantic, RRF), `vault_read` (an image path returns the image and its image document), `vault_history` (versions and a diff), `images_undescribed` (images still without words — the client looks and writes), `vault_visibility` (share a personal document / take one back), `vault_list`, `graph_lint`, `graph_suggest_links`, `vault_propose`, `vault_proposals`, `vault_promote`, `vault_write`, `vault_changes`, `vault_me` (your documents, remarks on them, proposals citing them, your inbox, plus a link to the app's My desk page), `inbox_send` / `inbox_list` / `inbox_reply` (ask a teammate's agent a question or hand it a task; it answers with that person's own context), `members_list`, `member_remember`, `member_report`; prompt `member`. Writes trigger member reactions exactly like saves from the app.
