...
try:
    ...
    print("ERROR: beautifulsoup4 not installed. pip install beautifulsoup4 lxml --break-system-packages")
    sys.exit(1)

try:
    import markdownify as md_lib
except ImportError:
    print("ERROR: markdownify not installed. pip install markdownify --break-system-packages")
    sys.exit(1)
...
def parse_meta(soup: BeautifulSoup) -> tuple:
    """Extract created date, modified date and source URL from the meta div."""
...
def preprocess_confluence(soup: BeautifulSoup, stem: str,
    ...
    """
    Confluence-specific tag preprocessing:
    - ac:image → replaced with img tags (handled by markdownify)
    - Remove/unwrap unnecessary ac:* tags
    Returns: list of copied image filenames
    """
    image_files = []
    img_counter = [0]

    # ── Optimization: pre-cache the files_dir listing (avoids repeated exists() calls on mounted paths) ──
    ...
    def resolve_image(orig_filename: str) -> str:
        """
        Symlink or copy the image into the attachments folder.
        Returns the new filename ('' if it does not exist).
        """
        img_counter[0] += 1
        ext = Path(orig_filename).suffix.lower() or '.png'
        new_name = f"{stem}_{img_counter[0]}{ext}"

        # Look up the file in the cache (case-insensitive)
        ...
        if dst.exists():
            ...
            return new_name
        try:
            # Link via symlink (fast). Copy on failure
    ...
    # ac:image → img tag conversion (markdownify handles ![]())
    for ac_img in soup.find_all('ac:image'):
        ...
        if ri_att and ri_att.get('ri:filename'):
            ...
            if new_fname:
                # Replace with an img tag that markdownify recognizes
...
def postprocess_md(md: str) -> str:
    ...
    # Convert img links generated by markdownify into Obsidian ![[]] format
...
def convert_html(args: tuple) -> dict:
    ...
    # Remove the meta div and h1 (handled in the frontmatter)
    meta_div = soup.find('div', class_='meta')
    if meta_div:
        meta_div.decompose()
    ...
    # Confluence preprocessing + image copy
    image_files = preprocess_confluence(soup, stem, files_dir, attachments_dir)

    # HTML → MD conversion via markdownify (fast)
...
def main():
    ...
    print(f"\nConverting {len(html_files)} files, starting...")
