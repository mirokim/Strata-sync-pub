...
[웹 데모](https://strata-sync-nine.vercel.app) · [3분 시연과 시작 가이드](manual.md) · [설명서 HTML](manual.html)
...
> 📘 **사용 설명서**: [docs/manual.md](manual.md) · HTML 판 [docs/manual.html](manual.html) — 왜 만들었는지, 무엇이 있는지, 매일 어떻게 쓰는지.
...
```
툴: `vault_recall`(주제에 대해 팀이 아는 것 한 묶음), `vault_search`(BM25 + 시맨틱 RRF), `vault_read`(이미지 경로면 이미지 + 이미지 문서), `vault_history`(버전과 diff), `images_undescribed`(설명 없는 이미지 목록 — 클라이언트가 보고 써 넣음), `vault_visibility`(개인 문서 공개/회수), `vault_list`, `graph_lint`, `graph_suggest_links`, `vault_propose`, `vault_proposals`, `vault_promote`, `vault_write`, `vault_changes`, `vault_me`(내 문서·내 문서에 온 반응·인용한 제안·받은 편지함 + 앱의 '내 책상' 링크), `inbox_send`/`inbox_list`/`inbox_reply`(팀원의 에이전트에게 질문·작업을 보내고 그 사람 맥락으로 답을 받음; 작업은 `chain`으로 다음 사람에게 릴레이), `radar_check`(모순 레이더 — 팀원이 쓴 문서와 충돌하면 편지함 질문으로), `members_list`, `member_remember`, `member_report`; 프롬프트 `member`. 쓰기는 앱에서 저장한 것과 똑같이 AI 팀원 반응을 일으킵니다.
...
```
Tools: `vault_recall` (what the team knows about a topic, one bundle), `vault_search` (BM25 + semantic, RRF), `vault_read` (an image path returns the image and its image document), `vault_history` (versions and a diff), `images_undescribed` (images still without words — the client looks and writes), `vault_visibility` (share a personal document / take one back), `vault_list`, `graph_lint`, `graph_suggest_links`, `vault_propose`, `vault_proposals`, `vault_promote`, `vault_write`, `vault_changes`, `vault_me` (your documents, remarks on them, proposals citing them, your inbox, plus a link to the app's My desk page), `inbox_send` / `inbox_list` / `inbox_reply` (ask a teammate's agent a question or hand it a task; it answers with that person's own context; tasks relay along `chain`), `radar_check` (contradiction radar — collisions with teammates' documents become inbox questions), `members_list`, `member_remember`, `member_report`; prompt `member`. Writes trigger member reactions exactly like saves from the app.
