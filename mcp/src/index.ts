#!/usr/bin/env node
/**
 * Sandbox Map MCP Server — Entry point.
...
async function main() {
  ...
  console.error(`[sandbox-map] config loaded (${Date.now() - t0}ms)`)

  // 2. Connect transport IMMEDIATELY — Claude Code can start sending requests
  const server = createServer()
  const transport = new StdioServerTransport()
  await server.connect(transport)
  console.error(`[sandbox-map] ⚡ ready (${Date.now() - t0}ms)`)

  // 3. Lazy-load vault in background — search/graph tools work once this resolves
  if (config.vaultPath) {
    reloadVault(config.vaultPath).then(stats => {
      console.error(`[sandbox-map] vault loaded: ${stats.docCount} docs, ${stats.nodeCount} nodes, ${stats.linkCount} links (${Date.now() - t0}ms)`)
    }).catch(e => {
      console.error(`[sandbox-map] vault load failed:`, e)
    })
  }
}
...
main().catch((e) => {
  console.error('[sandbox-map] fatal:', e)
  process.exit(1)
})
