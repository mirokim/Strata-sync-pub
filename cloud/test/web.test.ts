...
describe('handleMcpRequest', () => {
  ...
  it('answers initialize, tools/list and tools/call as plain JSON without a session', async () => {
    ...
    expect(list.result.tools.map(t => t.name).sort()).toEqual([
      'graph_lint', 'graph_suggest_links', 'images_undescribed', 'inbox_list', 'inbox_reply', 'inbox_send', 'member_remember', 'member_report', 'members_list', 'radar_check', 'radar_report', 'vault_changes', 'vault_history', 'vault_list', 'vault_me', 'vault_promote', 'vault_proposals', 'vault_propose', 'vault_read', 'vault_recall', 'vault_search', 'vault_visibility', 'vault_write',
    ])
