...
export async function handleMcpRequest(req: Request, deps: McpDeps): Promise<Response> {
  ...
  const server = new Server({ name: 'strata-sync-cloud', version: '0.5.0' }, {
    ...
    instructions: 'Team vault (shared brain). At the start of a session call vault_me: it lists questions and tasks teammates\' agents left for this user (answer them with inbox_reply using this user\'s own context) and replies that arrived. To ask or assign something to a specific teammate, use inbox_send with their name. Record ideas with vault_propose, never write documents directly unless asked.',
  })
