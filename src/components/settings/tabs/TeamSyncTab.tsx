...
export default function TeamSyncTab() {
  ...
  if (!api) {
    return <div style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>Team sync is available in the desktop app only.</div>
  }
  ...
  const test = async () => {
    ...
    try {
      ...
      setTestResult(r.ok
        ? { ok: true, text: `Connected — ${r.files} file${r.files === 1 ? '' : 's'} on the server (seq ${r.head})` }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18, padding: '2px 0' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 2, background: 'var(--color-bg-surface)', border: '1px solid var(--color-border)' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: enabled ? 'var(--color-accent)' : 'var(--color-text-primary)' }} data-testid="sync-status-title">
            {!enabled ? 'Team sync off'
              : status?.inFlight ? 'Syncing…'
              : status?.lastError ? 'Sync error'
              : `In sync · last ${relative(status?.lastSyncAt ?? null)}`}
          </div>
          <div style={{ fontSize: 11, color: 'var(--color-text-muted)', marginTop: 2 }}>
            {state?.vaultPath ? state.vaultPath : 'No vault open'}
            {enabled && status ? ` · ${status.pending} pending · seq ${status.lastSeq}` : ''}
          </div>
      ...
      <div>
        ...
        <div style={card}>
          <div>
            ...
            <div style={hint}>The Cloudflare Worker in <code>cloud/</code>. One deployment per team.</div>
          </div>
          <div>
            <label style={label}>Team token</label>
            <input value={token} onChange={e => setToken(e.target.value)} type="password" placeholder="shared team token" style={fieldInputStyle} spellCheck={false} data-testid="sync-token" />
            <div style={hint}>Stored encrypted on this machine. Everyone on the team uses the same token.</div>
          </div>
          <div>
            <label style={label}>Your name</label>
            <input value={author} onChange={e => setAuthor(e.target.value)} placeholder="shown on conflict copies and in file history" style={fieldInputStyle} data-testid="sync-author" />
          </div>
      ...
      {status && status.conflicts.length > 0 && (
        <div>
          <div style={sectionLabel}>Conflicts kept as copies</div>
              ...
              <div key={`${c.path}-${c.at}-${i}`} style={{ display: 'flex', gap: 8, alignItems: 'flex-start', fontSize: 12 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--color-text-muted)', wordBreak: 'break-all' }}>
                    {c.remoteAuthor ? `${c.remoteAuthor}'s version kept the name; ` : ''}your version saved as <code>{c.keptAs}</code>
                  </div>
                </div>
              </div>
            ))}
      ...
      {status && status.errors.length > 0 && (
        <div>
          <div style={sectionLabel}>Not synced</div>
      ...
      <div style={{ fontSize: 11, color: 'var(--color-text-muted)', lineHeight: 1.6 }}>
        How it works: edits are uploaded a couple of seconds after you save; other people's changes are pulled every 30 seconds.
        If two people change the same file, the server version keeps the file name and yours is saved next to it as a
        <code> (conflict …)</code> copy — nothing is overwritten. Files and folders starting with a dot stay on this machine.
      </div>
    </div>
  )
}
