    ...
    <div className="flex flex-col gap-7">
      ...
      <section>
        <div className="flex items-center gap-1.5 mb-3">
          ...
          <h3 className="text-xs font-semibold" style={{ color: 'var(--color-text-secondary)' }}>{t('Language')}</h3>
        </div>
        <div className="flex gap-2">
          {LANGUAGES.map(lang => {
            ...
            return (
              ...
              >
                {lang.id === 'system' ? t('System') : lang.label}
              </button>
            )
          })}
        </div>
      ...
      {/* Theme */}
      <section>
        <h3 className="text-xs font-semibold mb-3" style={{ color: 'var(--color-text-secondary)' }}>{t('Theme')}</h3>
      ...
      {/* Editor */}
      <section>
        <h3 className="text-xs font-semibold mb-3" style={{ color: 'var(--color-text-secondary)' }}>{t('Editor')}</h3>
        <div
          className="flex items-center justify-between px-3 py-2.5 rounded-lg"
          style={{ background: 'var(--color-bg-surface)', border: '1px solid var(--color-border)' }}
        >
          <div>
            <div className="text-xs" style={{ color: 'var(--color-text-primary)' }}>{t('Default Edit Lock')}</div>
            <div className="text-[11px] mt-0.5" style={{ color: 'var(--color-text-muted)' }}>
              {t('Open documents in read-only mode by default')}
            </div>
          </div>
          <button
            role="switch"
      ...
      <section>
        ...
        >
          <div>
            <div className="text-xs" style={{ color: 'var(--color-text-primary)' }}>{t('Show Node Labels')}</div>
            <div className="text-[11px] mt-0.5" style={{ color: 'var(--color-text-muted)' }}>
              {t('Always show node names in the graph')}
            </div>
          </div>
          <button
            role="switch"
      ...
      <section>
        <div className="flex items-center gap-1.5 mb-3">
          ...
          <h3 className="text-xs font-semibold" style={{ color: 'var(--color-text-secondary)' }}>{t('Document Rendering Quality')}</h3>
        </div>
        <div className="grid grid-cols-3 gap-2">
      ...
      <section data-testid="vault-section">
        <h3 className="text-xs font-semibold mb-3" style={{ color: 'var(--color-text-secondary)' }}>{t('Vault Path')}</h3>
        <VaultSelector />
      </section>
    </div>
  )
