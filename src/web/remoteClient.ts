...
export class RemoteClient {
  ...
  /** Questions and tasks between teammates — see cloud/src/inbox.ts */
  async inbox(status?: InboxStatus): Promise<InboxView> {
    const res = await this.request(`/v1/inbox${status ? `?status=${status}` : ''}`)
